...
This project require the implementation of different functions, centered around the calculation of the shortest path on a hex-grid map with variable costs, focused on time and space efficiency.

PROJECT SPECIFICATIONS:
Implementing the following functions, all applying to a rectangular map divided into hexagonal grids.
...
- change_cost(y, x, v, r): given a center poing of y,x coordinates, changes the cost of all hexagons within a radius of r according to a formula using v, between -10 and 10, and proportional to the hexagon's distance from the center. The cost of every air route going out also changes. Costs are limited from 0 to 100.
...
OBSERVATIONS:
- The hexagonal grid is often used in mapping, because it allows to move diagonally in addition to horizontally/vertically. One possibility to deal with this type of grid is using cubic coordinates, but I decided not to for simplicity: by using offset instead I could determine the position of the hexagon within the rectangular map (or matrix) in a more intuitive way. With this approach, the calculation of the neighboring hexagons depends on the starting one's position, in particular if it's placed on a even or odd row.
...
DECISIONAL CHOICES:
Because of the limit on overall complexity I decided to trade cleanliness for better and faster performance: instead of using a struct (matrix of hexagons was my first idea), which would require frequent access to non contiguous parts of the memory, I decided to keep all my datas in arrays. I also opted for Dial's algorithm, which optimizes Dijkstra's algorithm, since there was a limited range of edge weights.
...
I also decided not to store pointers to the neighboring hexagons, even if the access to these is required often, because of the space limitation. Instead, I chose to calculate their coordinates each time because such computation is quite easy when storing a small supporting array based on the hexagon's type.
